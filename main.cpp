...
void KasirFrame::SimpanTransaksiKeLog(double total, double bayar, double kembalian) {
    // Query untuk menyimpan total transaksi
    const char* insertSQL = 
        "INSERT INTO transaksi_log (total, bayar, kembalian, tanggal) "
        "VALUES (?, ?, ?, ?);";
    ...
    sqlite3_bind_double(stmt, 2, bayar);
    sqlite3_bind_double(stmt, 3, kembalian);

    // Format tanggal menggunakan wxDateTime
    wxDateTime now = wxDateTime::Now();
    wxString tanggal = now.FormatISODate() + " " + now.FormatISOTime();
    sqlite3_bind_text(stmt, 4, tanggal.ToUTF8().data(), -1, SQLITE_STATIC);

    // Eksekusi query
    int result = sqlite3_step(stmt);
    if (result != SQLITE_DONE) {
        wxLogError("Gagal menyimpan transaksi: %s", sqlite3_errmsg(db));
    }
